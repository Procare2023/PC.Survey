...
namespace PC.Web.Initializer
{
    public class DbInitializer : IDbInitializer
    {
        ...
        {
            if (_roleManager.FindByNameAsync(PC.Services.Core.Helper.Consts.SD.Admin).Result == null)
            {
                _roleManager.CreateAsync(new IdentityRole(PC.Services.Core.Helper.Consts.SD.Admin)).GetAwaiter().GetResult();
                _roleManager.CreateAsync(new IdentityRole(PC.Services.Core.Helper.Consts.SD.User)).GetAwaiter().GetResult();
            }
            else { return; }
            ...
            var temp1 = _userManager.AddClaimsAsync(adminUser, new Claim[] {
                ...
                new Claim(JwtClaimTypes.Role,PC.Services.Core.Helper.Consts.SD.Admin),
            }).Result;
